											...
											<c:if test="${not empty list }">
												<c:forEach items="${list }" var="courier">
													<tr>														
														...
														<td>${courier.cuPwd}</td>
														<td>${courier.cuTel}</td>
														<td>${courier.provice}${courier.city}${courier.region}${courier.street}</td>
														<td>
	...
	<script type="text/javascript">
		...
		function editInfo(obj,type){
			$.getJSON("<%=basePath%>changecourier.html",function(data){
				$('#brandselectp').empty();
				$.each(data,function(){
					$('#brandselectp').append("<option value="+this.beachid+">"+this.province+this.city+this.region+this.street+"</option>");
				});
			...
			if(type==0){
				...
				$("#editForm").attr("action","<%=basePath%>updatecourier.html");
